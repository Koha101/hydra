...
BRIDGE_DIR="$CONFIG_DIR/plugins/cache/${HYDRA_MARKETPLACE:-claude-plugins-official}/discord"
if ls "$BRIDGE_DIR"/*/server.ts >/dev/null 2>&1; then
  ok "bridge plugin present in config dir"
else
  bad "bridge plugin NOT in $CONFIG_DIR — sessions can't reach the daemon. Install: claude plugin install discord@${HYDRA_MARKETPLACE:-claude-plugins-official}"
fi
