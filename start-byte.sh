...
DEST="$CONFIG_DIR/plugins/cache/${HYDRA_MARKETPLACE:-claude-plugins-official}/discord/0.0.4/server.ts"
if [ ! -f "$SRC" ]; then
...
tmux new-session -d -s "$SESSION" \
  "cd '$CWD' && export DAEMON_SOCK='$SOCK' && export CLAUDE_CONFIG_DIR='$CONFIG_DIR' && export CHAT_PLATFORM=$CHAT_PLATFORM && $AUTH_EXPORT caffeinate -i claude --model 'claude-opus-4-8[1m]' --channels plugin:discord@${HYDRA_MARKETPLACE:-claude-plugins-official} \
  \"$PROMPT\""
