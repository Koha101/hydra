...
mcp.setRequestHandler(ListToolsRequestSchema, async () => {
  ...
  const fallback = [
    ...
    // Session management tools (main session only — daemon rejects otherwise)
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          model: { type: 'string', description: 'Optional model ID or Claude alias.' },
          provider: { type: 'string', enum: ['claude', 'codex'], description: 'AI provider. Defaults to Claude.' },
          phase_budget: { type: 'string', description: 'Optional maximum session lifetime, e.g. "20m" or "1h".' },
        },
        required: ['topic'],
      },
    },
    {
      name: 'list_sessions',
      description: 'List all active sessions. Main session only.',
      inputSchema: {
        type: 'object',
        properties: {},
      },
    ...
    {
      name: 'kill_session',
      description: 'Kill a running session by session ID or thread ID. Main session only.',
      inputSchema: {
        type: 'object',
        properties: {
