...
| **Delivery wake** — after a bridge reconnect CC stages inbounds as an un-submittable queued widget; daemon nudges idle+stuck sessions (kill-switch `HYDRA_DELIVERY_WAKE=0`); `/model` answers CC's switch-confirm modal | FIX | `daemon/router.ts`, `daemon/commands/session-config.ts` | CC auto-flushes queued channel notifications after reconnect. |
| **Gate approval bridge** — posts Allow once / Allow for session / Deny buttons for the external gate's 'ask' requests, writes HMAC-signed scoped grants the hook consumes | REQ | `daemon/gate-approval.ts` (new), `daemon/permission.ts`, `daemon.ts` | Keep — pairs with the external `~/.claude-hydra/gate` hook. |
| **resolveConfig env-ordering fix** — configDir/spawnCwd resolved after the `.env` sourcing (unsourced `hydra restart` used to boot the daemon on `~/.claude`, spawning ungated sessions) | FIX | `cli/helpers.ts` | PR upstream — not deployment-specific. |
