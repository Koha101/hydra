...
export function buildCodexForkPrompt(p: PromptParams & { originFrom: string }): string {
  return [
    `You are ${p.tmuxName}, a Codex session forked from ${p.originFrom}.`,
    `Continue from the inherited conversation while focusing on this topic: ${p.topic}`,
    `This is a separate Hydra Discord thread; the parent conversation remains unchanged.`,
    `Your working directory is the workspace Hydra started you in. Follow its loaded CLAUDE.md instructions.`,
    `Your final response is delivered directly to Discord, so briefly identify the forked focus and then proceed with the work.`,
    `Do not attempt to call Hydra or Discord MCP tools; the bridge sends your final response for you.`,
  ].join('\n')
}
