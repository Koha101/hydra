...
export function designPersonaPrompt(opts: {
  ...
  return [
    ...
    `3. Post 1-3 clarifying questions you need answered to make a good proposal from your lens`,
    `4. Tag with: \`[${persona}→questions]\``,
    `5. If you have NO questions (context is sufficient), post \`[${persona}→questions]\` followed by "No questions."`,
    ``,
    `**Phase 2 — Proposal (WAIT for answers first):**`,
    `After your questions are answered (arrives as a notification), post your proposal:`,
    `- Tag with: \`[${persona}→thread]\``,
    ...
    `- Form your proposal INDEPENDENTLY — do NOT read other personas' proposals`,
    ``,
    `After proposing, **WAIT**. You may be asked to refine your position in a later phase.`,
  ].join('\n')
}
