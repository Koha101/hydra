...
export async function handleCommandsIntercept(msg: InboundMessage): Promise<void> {
  ...
  const text = [
    ...
    '• `build [N] [task]` — build mode (default 3 rounds): owner implements, critic reviews correctness + tests. N = max rounds. Type `build 2` again after completion to continue.',
    '• `build-wt: <repo> [N] [task]` — build in an isolated git worktree (e.g. `build-wt: options_bot 3 implement ticket 1`)',
    '• `cancel build` — cancel an in-progress build',
