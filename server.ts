...
async function handleInbound(msg: Message): Promise<void> {
  ...
  //
  // Race/dedup guards:
  //  1. If the message already has a thread attached (msg.hasThread), reuse
  //     msg.thread.id instead of calling startThread again. Discord returns
  //     "A thread has already been created for this message" (HTTP 400) if
  //     you try to create a second one.
  //  2. Multiple bot instances can race on the same messageCreate event
  //     (happens when several sessions share a token). The winner creates
  //     the thread; losers catch the "already created" error and fall back
  //     to looking up the existing thread via fetch/client cache.
  //  3. Never post thread-creation errors into the channel — they're debug
  //     noise visible to humans. stderr only.
  if (!msg.channel.isDMBased() && !msg.channel.isThread()) {
    ...
    if (policy?.threadReply) {
      ...
      // Fast path: message already has a thread attached — reuse it.
      if (msg.hasThread && msg.thread) {
        ...
        try {
          ...
          process.stderr.write(
            `discord channel: startThread failed (channel type: ${msg.channel.type}): ${errMsg}\n`,
          )
          // Race recovery: if another instance already created the thread for
          // this message, re-fetch the message and pick up the existing thread.
          // Fall through to channel reply if still unavailable.
          try {
            ...
            process.stderr.write(
              `discord channel: thread race recovery fetch failed: ${reErr instanceof Error ? reErr.message : String(reErr)}\n`,
            )
          }
        }
      }
  ...
  // Thread context: if the message is in a thread, fetch the starter message
  // so Claude knows what the thread is about.
